...
public class DDSLineReader {
	...
	private boolean ignoreAlpha; //added for NFS models that sometimes use alpha sometimes not -needeka
	...
	public void decodeLine(byte[] bytes, DDSHeader ddsHeader, byte [][] banks, int width, int y, boolean ignoreAlpha) throws IOException{
	...
	private void decodeDXT(byte[] bytes, DDSHeader ddsHeader, byte [][] banks, int width, int y) throws IOException{
		...
		if (ddsHeader.getPixelFormat().isNormal()) {
			...
			for (int x = 0; x < width; x++) {
				...
				banks[BANK_ALPHA][x] = ignoreAlpha ? (byte)0xff : linesColor[lineNumber][x][BANK_ALPHA];
			}
		}
		lineNumber++;
	}
