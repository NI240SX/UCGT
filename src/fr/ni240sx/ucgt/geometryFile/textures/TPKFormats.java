...
public class TPKFormats extends Block{
	
	TPK tpk;
	
	@Override
	public BlockType getBlockID() {return BlockType.TPK_Formats;}
		
	public TPKFormats(ByteBuffer in, TPK tpk) {
		var blockLength = in.getInt();
		var blockStart = in.position();
		
		this.tpk = tpk;
		for (var t : tpk.textures) {
			t.readHeaderPart2(in); //length 24
		}
		in.position(blockStart+blockLength);
	}
	
	public TPKFormats(TPK tpk) {
		this.tpk = tpk;
	}
