...
//        MenuItem fileGenerateNew = new MenuItem("Generate new");
        ...
        fileLoad.setOnAction(e -> fileLoad(primaryStage));
        fileLoad.setAccelerator(KeyCombination.keyCombination("Ctrl+O"));
        fileNew.setOnAction(e -> fileNew(primaryStage));
...
//        fileGenerateNew.setAccelerator(KeyCombination.keyCombination("Ctrl+N"));
        fileSave.setOnAction(e -> fileSave(e));
        fileSave.setAccelerator(KeyCombination.keyCombination("Ctrl+S"));
        fileExit.setOnAction(e -> fileExit(primaryStage));
        ...
        editUndo.setOnAction(e -> Undo.undo());
        editUndo.setAccelerator(KeyCombination.keyCombination("Ctrl+Z"));
        editRedo.setOnAction(e -> Undo.redo());
...
//        CheckMenuItem dbSortAttributesExport = new CheckMenuItem("Sort attributes at export");
        ...
        dbOptimizeAttributes.setOnAction(e -> dbOptimizeAttributes());
        dbSortAttributes.setOnAction(e -> dbSortAttributes(e));
...
//        	new Alert(Alert.AlertType.WARNING, "Not implemented").show();
//        });
        dbChangeName.setOnAction(e -> dbChangeName(primaryStage, e));
        ...
        dbTrimToGeom.setOnAction(e -> dbTrimToGeom(e));
        ...
        partAdd.setOnAction(e-> partAdd(e));
...
//    	buttonCopy.setTooltip(new Tooltip("Simply duplicate selected part(s) (Ctrl+C)"));
        ...
        kitSaveTemplate.setOnAction(e -> kitSaveTemplate(e));
        kitDelete.setOnAction(e -> kitDelete());

        partsHoods.setOnAction(e -> {
        	partsHoods();
        	e.consume();
        });
        partsSpoilers.setOnAction(e -> {partsSpoilers();
	    	e.consume();
        });
...
//        attributesDisplay.setMinWidth(root.getWidth()/2);
	...
	@SuppressWarnings("unused")
	public static void dbTrimToGeom(ActionEvent e) {
		ButtonType sure = null;
		if (!disableWarnings) sure = new Alert(Alert.AlertType.WARNING, "Are you sure you want to trim this DB ? Backup recommended.", ButtonType.NO, ButtonType.YES).showAndWait().orElse(ButtonType.NO);
		if (ButtonType.YES.equals(sure) || disableWarnings) {
			...
			try {
		    	...
		    	bb.position(180); //skips common stuff at the beginning of the file
		    	int partsListLength = bb.getInt();
		    	int partsListBeginning = bb.position();

		    	var foundParts = new ArrayList<DBMPPart>();
		    	while (bb.position() < partsListBeginning + partsListLength) {
		    		int partHash = bb.getInt();
		    		bb.getInt();
		    	...
		    	// maps slotID and isWidebody to max kit
		    	var minMax = new HashMap<Pair<PartUndercover, Boolean>, Integer>();
		    	for (var p : foundParts) {
		    		PartUndercover id = ((AttributeCarPartID) p.getAttribute("PARTID_UPGRADE_GROUP")).ID;
		    		if (minMax.get(new Pair<>(id, p.isWidebody)) == null) {
		    			minMax.put(new Pair<>(id, p.isWidebody), p.kitnumber);
		    		} else {
		    			if (minMax.get(new Pair<>(id, p.isWidebody)) < p.kitnumber) { //value is maximum
		    				minMax.put(new Pair<>(id, p.isWidebody), p.kitnumber);
		    			}
		    		}
		    	}
		    	
				ArrayList<DBMPPart> toRemove = new ArrayList<>();
				for (var p : mainDBMP.dBMPParts) {
		    		PartUndercover id = ((AttributeCarPartID) p.getAttribute("PARTID_UPGRADE_GROUP")).ID;
		    		if (minMax.get(new Pair<>(id, p.isWidebody)) != null) {
		    			if (minMax.get(new Pair<>(id, p.isWidebody)) < p.kitnumber) { //value is maximum
		    				toRemove.add(p);
//			    			System.out.println("Gotta remove "+p.displayName);
		    			} 
		    		} else {
//		    			System.out.println("No mapping found for "+p.displayName);
		    			toRemove.add(p);
		    		}
				}
...
//					System.out.println("Removing part : "+p.displayName);
	...
	public static void dbFixNameOffsets() {
		for (DBMPPart p : mainDBMP.dBMPParts) {
	...
	@SuppressWarnings("unused")
	public static void dbChangeName(Stage primaryStage, ActionEvent e) {
	...
	public static void dbSortPartsByName2() {
	...
	public static void dbSortPartsByName() {
	...
	public static void partCopyAdvanced() {
		Stage st = new Stage();
	...
	@SuppressWarnings("unused")
	public static void partAdd(ActionEvent e) {
	...
	public static void partsSpoilers() {
		Stage st = new Stage();
	...
	@SuppressWarnings("unused")
	public static void kitSaveTemplate(ActionEvent e) {
		if (partsDisplay.getSelectionModel().getSelectedItem()!=null) {	
			String kit = ((AttributeTwoString)partsDisplay.getSelectionModel().getSelectedItem().getAttribute("PART_NAME_OFFSETS")).value1;
			FileChooser fc = new FileChooser();
	...
	public static void kitExhausts() {
		Stage st = new Stage();
	...
	@SuppressWarnings("unused")
	public static void kitGenerate(ActionEvent e) {
		Stage st = new Stage();
	...
	public static void fileExit(Stage primaryStage) {
		ButtonType sure = null;
		if (!disableWarnings)  sure = new Alert(Alert.AlertType.INFORMATION, "Are you sure you want to exit ? Any unsaved changes will be lost.", ButtonType.NO, ButtonType.YES).showAndWait().orElse(ButtonType.NO);
		if (ButtonType.YES.equals(sure) || disableWarnings) {
			primaryStage.close();
	...
	public static void fileLoad(Stage primaryStage) {
		ButtonType sure = null;
...
//						new Alert(Alert.AlertType.INFORMATION, "Nothing to load", ButtonType.OK).show();
	...
	public static void fileNew(Stage primaryStage) {
		ButtonType sure = null;
