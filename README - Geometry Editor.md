...
- glTF (.gltf, .glb): recommended file format for full support
- Wavefront (.obj): works fine for cars, do not use for world models with more than one UV channel
- ZModeler 2 (.z3d)
...
- NFSC cars, textures and generic parts: PC partial read (EXPERIMENTAL)
- NFSW cars, textures and generic parts: PC partial read (EXPERIMENTAL)
...
## Model preparation
UCGT takes models in the orientation of the game: +X front, +Z top for OBJ. Extract existing models to give you templates!
...
- `compile <GLTF/OBJ/Z3D/folder> <BIN output>` - compile a single 3D model or a folder and its/their corresponding config(s) to a BIN file containing one geometry per 3D model. If DDS textures are found in the folder, also compiles the TEXTURES.BIN.
- `decompile <BIN source> [GLTF/OBJ/Z3D output]` - extract a BIN file containing a single geometry to a 3D model and a configuration file. If an associated TEXTURES.BIN exists, also extracts the textures.
...
Decompiling a Geometry will generate a 3D model in the chosen format and an INI configuration file to compile it back.
...
### Settings
This part uses the keyword `SETTING` ; several settings can be written next to each other on a single line. Most settings have the best value selected by default for a car model. The possible settings are the following :

_Base_
- `CarName` - sets the car name in the output Geometry. Ignore for world models, wheels or any other model without a single name used across all parts.
...
_Compression_
- `UseMultithreading` - use single-threaded or multi-threaded compression for car parts, multi-threaded significantly decreases the time needed to compile, especially when using RefPack and high compression settings. Possible values: true or false.
- `CompressionType` - set the compression type used for parts. Possible values : RefPack/RFPK, JDLZ, RawDecompressed/RAWW. JDLZ is faster but RefPack High yields lighter files (RefPack is only supported by UC).
- `CompressionLevel` - set the compression level used. Has no effect if the compression type isn't RefPack. High or better recommended when your model is finalized. Possible values : Minimum, Low, Medium, High, VeryHigh, Ultra, Maximum.

_Optimization_
- `RemoveInvalid` - removes invalid parts that don't fit with the game cars' naming scheme. Possible values : true or false. Due to UCGT's flexible approach, this is very basic and will only remove parts without a KIT and valid LOD.
- `RemoveUselessAutosculpt` - removes useless _T0 parts when there's no actual morph zones to reduce file size. Possible values : true or false.
- `OptimizeMaterials` - removes some unnecessary data in material declarations to help reduce file size. Possible values : true or false.
- `SortAllByName` - sorts parts, markers, materials etc. by name. Possible values : true or false.
- `CopyMissingLODs` - if one or multiple LODs are missing, copies the mesh from a higher LOD. Possible values : true or false.
- `MakeLodD`/`CopyLodD`/`CopyLastLod` - if the KIT00 lod D is missing, copies it from lod C. It is highly advised to make a lod D manually ; setting this option to true will allow multiple lod C parts to be copied as lod D, making it unnecessarily heavy and potentially causing issues. Respectively lods E and D for other games. Possible values : true or false.

_Mesh_
- `VertexColors` - import vertex colors (baked AO) from external model, generate them with an integrated method, or don't include them. Possible values : Import, Generate/Calculate, No.
- `Tangents` - whether to calculate vertex tangents for normalmapped materials. High recommended, for Manual see the Materials section. Possible values : Off/No/None, Low/Optimized/ByShaderUsage, High/ByTextureUsage, Manual/PerMaterial, All/On/ForceCalculate.
- `FlipV` - whether to flip the vertical axis on UV maps. Use in case your textures end up upside down. Possible values : true or false.

_Vertex colors calculation_
- `VertexColorBrightnessMin` and `VertexColorBrightnessMax` - controls the brightness range for fake AO. Possible values from 0-255, default 0 and 255.
- `VertexColorBrightnessFactor` - controls the brightness factor for fake AO based on normals. Possible integer values, default 150.
- `VertexColorBrightnessNormalOffset` - controls the normal offset for fake AO. This is added to the normal's Z value. Possible decimal values, default 0.8.
- `VertexColorBrightnessMinWheel` and `VertexColorBrightnessMaxWheel` - controls the brightness range for fake AO on wheels. Possible values from 0-255, default 0 and 255.
- `VertexColorBrightnessFactorWheel` - controls the brightness factor for wheel fake AO based on normals. Possible integer values, default 150.
- `VertexColorBrightnessNormalOffsetWheel` - controls the normal offset for fake AO on wheels. This is added to the normal's Z value. Possible decimal values, default 0.8.

_Experimental fixes_
- `ForceAsFix` - force-fixes autosculpt on the specified part if it doesn't get compiled correctly (this happens when there's the same amount of vertices on every morph zone but the welding is different). You can put this setting multiple times. Possible values: any part name.
- `FixAutosculptNormals` - attempts to fix normals on zero area triangles, greatly improving the look of some Autosculpt parts, including vanilla. Possible values: true or false.
- `ProtectModel` - removes plain text part names, effectively making it hard/tedious to steal your model. Possible values: true or false.
- `CheckModel` - performs various quality checks on the model and reports possible issues. Possible values: true or false.
...
- The material lighting and shader, in the format `MATERIAL_CAR_LIGHTING=Shader` with no space inbetween.
...
The material name, material lighting name and textures names are nonrestricted.

For UC cars, the possible shaders are the following.

Other shaders such as in UC world materials and models from earlier games can be found in data\shaderusages and data\shaderusages_legacy respectively.


_Regular_
- car aka Diffuse : use for most basic materials.
- car_a aka DiffuseAlpha/Alpha : use for basic materials with transparency.
- car_a_nzw : supposed to be transparency with no z-write, effectively doesn't seem to change anything.
- car_nm aka DiffuseNormal/Normal : use for basic materials with normal.
- car_nm_a aka DiffuseNormalAlpha/NormalAlpha : use for basic materials with both alpha and normal.

_Skinnable_
- car_v aka DiffuseSwatch/Swatch : do not use, gives broken shading.
- car_nm_v_s aka DiffuseNormalSwatch/NormalSwatch : use for carskin and carbonfiber, probably anything skinnable/damageable.
- car_nm_v_s_a aka DiffuseNormalSwatchAlpha/NormalSwatchAlpha : unused in vanilla models, probably the same as above with transparency.

_Emissive_
- car_si aka DiffuseSelfIllumination/DiffuseGlow/SelfIllumination/Glow : use for head- and taillights.
- car_si_a aka DiffuseSelfIlluminationAlpha/DiffuseGlowAlpha/SelfIlluminationAlpha/GlowAlpha : use for brakelight glass.

_Traffic_
- car_t aka TrafficDiffuse/Traffic : use for basic materials on traffic cars.
- car_t_a aka TrafficDiffuseAlpha/TrafficAlpha : use for transparent traffic car materials.
- car_t_nm aka TrafficDiffuseNormal/TrafficNormal : use for traffic car materials with normals, including carskin.

In most cases, you can invert names in the aliases, the compiler should still understand it (e.g. AlphaNormal works the same as NormalAlpha).
...
- DIFFUSE: simply use the texture as diffuse (if the shader usage is car_a/DiffuseAlpha, the alpha channel is taken from this texture too).
- NORMAL: use as normalmap.
- AMBIENT: use as paintable/skinnable vinyl placeholder.
- OPACITY: use as opacity map.
...
Texture usages are not necessary for legacy games.
...
Materials to be used must have one of the following shaders: `car`, `car_a`, etc.
...
Materials to be used must have one of the following shaders: `car_t`, `car_t_a` and `car_t_nm`.
...
Materials in world and NIS models are slightly different from cars, most of them are governed by a shader and not so much by material lighting parameters like cars.